...
public class DialogueUIElement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (continueButton)
        {
            if (GameStateManager.dialogueManager.playingChoices) continueButton.interactable = false;
            else
            {
                continueButton.interactable = !GameStateManager.dialogueManager.autoDialogue;
            }
        }
    }
    ...
    public virtual (GameObject, GameObject) Create(GameObject original, Transform parent, CarController car)
    {
        ...
        {
            script.car = car;
            script.continueButton.onClick.AddListener(() => OnClick(script.continueButton));
    ...
    {
        // Disables this button once clicked once
        button.interactable = false;

        GameStateManager.dialogueManager.ContinueButton();
    }
}